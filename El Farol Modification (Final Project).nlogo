...
turtles-own [
  ...
  ;; Ivan Prskalo:
  apathy          ;; chance that person will not care and either go or not go at random
]
...
to setup
  ...
  create-turtles 100 [
    ...
    ;; Ivan Prskalo:
    set apathy random-float apathy-max
...
to go
  ...
  ask turtles [
    ...
    ;;Ivan Prskalo (Next 3 Lines. The If Statement):
    if random-float 1 <= apathy [
      set attend? one-of [ true false ]
    ]
  ]
